 ...
 *
 * Lambda@Edge does not support environment variables, so bucket name and
 * region are derived from the CloudFront origin config in the event itself.
 */
exports.handler = async (event) => {
  const request = event.Records[0].cf.request;
  const uri = request.uri;

  // Extract bucket name and region from the CloudFront S3 origin
  const origin = request.origin && request.origin.s3;
  const bucket = origin ? origin.domainName.split('.')[0] : null;
  const region = origin ? (origin.region || 'us-east-1') : 'us-east-1';
